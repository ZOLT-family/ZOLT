...
>
> **Update 23 Sep 2026 (sore, "token terakhir, lanjut step berikutnya"):** (1) **API publik** `GET /api/markets` (+`?id=`): 24 pasar terakhir, pool, implied odds, deadline, calldata stake YES/NO — read-only, CORS terbuka, cache 5 detik; `config.json` ditulis builder dari record deploy (publik di `/config.json`). (2) **Link share per pasar**: tombol *share* di tiap pasar terbuka nyalin satu baris siap paste + `?m=ID`; buka `?m=ID` otomatis milih launch + window pasarnya. (3) **Keeper versi cloud**: `keeper/Dockerfile` + `railway.toml` + `keeper/package.json` (viem sendiri); belum dibuild (gak ada Docker di mesin ini). (4) `AGENTS.md` buat bot/agen. Token belum dilaunch — sesuai permintaan, paling akhir.
>
